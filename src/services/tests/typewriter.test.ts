...
describe("typeNext unit tests", () => {
  ...
  describe("no loop limit", () => {
    const phrases = ["Ke dil haare, pukaare", "Mann ja re, mana le"];
    const options = {
      typingSpeed: { numUnits: 1, timeMs: 100, startDelayMs: 10 },
      erasingSpeed: { numUnits: 1, timeMs: 50, startDelayMs: 5 },
    };

    test("Should invoke onNext indefinitely", () => {
      const progress = {
        phrase: "phrase",
        phraseIdx: 1,
        status: TypewriterStatus.Erasing,
      ...
      jest.advanceTimersByTime(1000000); //advance so much in the future
